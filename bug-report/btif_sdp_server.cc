...
int get_sdp_records_size(bluetooth_sdp_record* in_record, int count) {
  ...
  int records_size = 0;
  LOG_INFO("size_of records_size: %zu", sizeof(records_size));
  int i;
  for (i = 0; i < count; i++) {
    record = &in_record[i];
    ...
    LOG_INFO("record_size: %d, 0x%016x", records_size, records_size);
...
static int alloc_sdp_slot(bluetooth_sdp_record* in_record) {
  ...
  LOG_INFO("=====calculated size of in_record: 0x%016x", record_size);
  bluetooth_sdp_record* record = (bluetooth_sdp_record*)osi_malloc(record_size);
  LOG_INFO("=====going to copy input record from %p to %p", in_record, record);
  LOG_INFO("=====at least required size of in_record: 0x%016lx", (unsigned long) sizeof(bluetooth_sdp_record));
  ...
  LOG_INFO("=====copying SDP record done!");
