...
#define LOG_TAG "bt_os_allocator"
...
void* osi_malloc(size_t size) {
  LOG_INFO("=====requested allocation size: %zu or %#016lx", size, (unsigned long) size);
  size_t real_size = allocation_tracker_resize_for_canary(size);
  LOG_INFO("=====real size allocated after allocation tracker resize for canary: %zu or %#016lx",
           real_size, (unsigned long) real_size);
  void* ptr = malloc(real_size);
  CHECK(ptr);
  LOG_INFO("=====going to notify alloc tracker");
